...
const Player = () => {
  ...
  return (
    <div className='h-[10%] bg-black flex justify-between items-center text-yellow-300 px-4'>
        ...
        <div className='flex items-center gap-5'>
            <p className='text-red-500'>{time.currentTime.minute}:{time.currentTime.second}</p>
        <div ref={seekBg} onClick={seekSong} className='w-[60vw] max-w-[500px] bg-orange-400 h rounded-full cursor-pointer'>
            <hr ref={seekBar} className='h-1 border-none w-0 bg-red-800 rounded-full' />
        </div>
                <p className='text-red-500'>{time.totalTime.minute}:{time.totalTime.second}</p>
        </div>
        </div>
        ...
        <div className='w-20 bg-red-500 h-1 rounded cursor-pointer'>
